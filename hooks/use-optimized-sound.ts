      ...
      if (playPromise) {
        playPromise
          .then(() => {
            ...
            console.log(`Audio played successfully in ${endTime - startTime}ms`)
          })
          .catch((error) => {
...
export function useOptimizedAppSounds() {
  const clickSound = useOptimizedSound("https://hebbkx1anhila5yf.public.blob.vercel-storage.com/11L-Clic_de_bouton_girly-1754003907888-JLmi4woLnORJ8q2N7JzYlWZDfsP6Tv.mp3", {
    volume: 0.4,
    poolSize: 2,
  })

  const menuClickSound = useOptimizedSound("https://hebbkx1anhila5yf.public.blob.vercel-storage.com/girlyclick-m35wgd6n66cyCgXLprXDMXMNnmyAQY.mp3", {
    volume: 0.6,
